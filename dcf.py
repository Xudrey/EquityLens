"""Simple five-year DCF valuation model."""
...
def run_dcf(historical_data: pd.DataFrame, assumptions: DCFAssumptions) -> dict[str, Any]:
    """Project unlevered FCF and calculate fair value per share."""
    if assumptions.wacc <= assumptions.terminal_growth:
        raise ValueError("WACC must be greater than terminal growth.")
    latest = historical_data.sort_values("year").iloc[-1]
    ...
    terminal_fcf = float(projections["unlevered_fcf"].iloc[-1]) * (1 + assumptions.terminal_growth)
